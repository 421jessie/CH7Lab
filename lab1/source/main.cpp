...
int main()
{
	...
	stream1 = fopen("本機//桌面/CH7 lab/lab1/gradedata.txt", "r");
	stream2 = fopen("本機//桌面/CH7 lab/lab1/gradeoutput.txt", "w");
	...
	if (stream1 == NULL || stream2 == NULL) {
		...
		for (int i = 0; i < 5; i++) {
			...
			fscanf(stream1, "%s", &data[i].chinese);
			fscanf(stream1, "%s", &data[i].english);
			fscanf(stream1, "%s", &data[i].math);
